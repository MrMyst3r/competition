...
for i in range(0, len(y_test)-2):
    if (predicted[i+2][0]-predicted[i+1][0])*(y_test[i+1]-y_test[i]) > 0:
        adj_acc += 1
        
for i in range(0, len(y_test)-3):
    if (predicted[i+3][0]-predicted[i+2][0])*(y_test[i+1]-y_test[i]) > 0:
        m_adj_acc += 1
acc_perc = acc/(len(y_test)-1)*100
adj_acc_perc = adj_acc/(len(y_test)-2)*100
m_adj_acc_perc = m_adj_acc/(len(y_test)-3)*100
...
st.write(f'**이 모델은 <span style="color: red;">{adj_acc_perc}%</span>의 확률로 주가의 상승, 하락을 올바르게 예측한다.(1일 조정된 예측)**', unsafe_allow_html=True)
st.write(f'**이 모델은 <span style="color: red;">{m_adj_acc_perc}%</span>의 확률로 주가의 상승, 하락을 올바르게 예측한다.(2일 조정된 예측)**', unsafe_allow_html=True)
